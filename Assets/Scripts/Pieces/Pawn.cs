...
public class Pawn : Piece
{
    ...
    public override bool LegalMove(Tile targetTile)
    {
        ...
        {
            if (targetTile.m_coordinates.y == m_currentCoordinates.y + 1)
            {
    ...
    public override bool LegalAttack(Tile targetTile)
    {
        if (targetTile.m_coordinates.y == m_currentCoordinates.y + 1)     // Check if the target tile is one step forward
        {
            if (Mathf.Abs(targetTile.m_coordinates.x - m_currentCoordinates.x) == 1)      // Check if the target tile is one step diagonally
            {
                return true;
            }
        }
